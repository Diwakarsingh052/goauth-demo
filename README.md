...
| `DB_NAME` | Database name | `challenge_go` |
...
```sql
CREATE DATABASE challenge_go;
CREATE DATABASE challenge_go_test;  -- for running tests
```
...
**Integration tests** (requires MySQL `challenge_go_test` database):
```bash
