...
test: ## Run all tests (unit + integration if DB available)
	...
	@echo "Set TEST_DB_DSN env var or ensure challenge_go_test database exists."
...
setup: ## Create MySQL databases for development and testing
	...
	mysql -u $${DB_USER:-root} -e "CREATE DATABASE IF NOT EXISTS $${DB_NAME:-challenge_go};" 2>/dev/null || \
		echo "Could not create dev database. Check MySQL connection."
	mysql -u $${DB_USER:-root} -e "CREATE DATABASE IF NOT EXISTS challenge_go_test;" 2>/dev/null || \
...
migrate: ## Run database migration (also runs automatically on API start)
	...
	@echo "  mysql -u root challenge_go < migrations/001_init.sql"
