...
// GoogleAuth calls POST /api/auth/google.
func (c *APIClient) GoogleAuth(googleID, email, name, mode string) (*AuthResponse, error) {
	body := map[string]string{"google_id": googleID, "email": email, "name": name, "mode": mode}
	return c.doAuthRequest("POST", "/api/auth/google", body, "")
}
