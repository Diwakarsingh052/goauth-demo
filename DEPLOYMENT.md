...
```bash
cd /opt
git clone <YOUR_REPO_URL> challenge-go-cyaz
cd challenge-go-cyaz
```
...
docker compose -f docker-compose.prod.yml exec mysql \
  mysqldump -u root -p"${DB_PASSWORD}" challenge_go > backup_$(date +%Y%m%d).sql
```
...
0 2 * * * cd /opt/challenge-go-cyaz && docker compose -f docker-compose.prod.yml exec -T mysql mysqldump -u root -p"$(grep DB_PASSWORD .env | cut -d= -f2)" challenge_go | gzip > /opt/backups/db_$(date +\%Y\%m\%d).sql.gz
```
